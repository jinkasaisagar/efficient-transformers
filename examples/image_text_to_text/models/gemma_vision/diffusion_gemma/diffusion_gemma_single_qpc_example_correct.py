...
# text_only_prompt_text = "What are the seven continents? Answer in one sentence."
text_only_prompt_text = "What is diffusion based generative learning? Answer in one sentence."
text_only_prompt_text = "How to make pizza? Answer in one sentence."
...
parser.add_argument(
    ...
    choices=("local", "hf"),
    default="local",
    help="Sampler acceptance mode: cumulative local freezing or Hugging Face per-step re-noising.",
)
...
def _next_encoder_position():
    valid_positions = position_ids[position_ids >= 0]
    return int(valid_positions.max()) + 1 if valid_positions.size else 0


def _denoise_canvas(block_index):
    canvas_start = _next_encoder_position()
    canvas = np.random.randint(0, vocab_size, size=(1, canvas_length), dtype=np.int64)
    new_canvas = canvas.copy()
    canvas_pos = np.arange(canvas_start, canvas_start + canvas_length, dtype=np.int64).reshape(1, -1)
    ...
    for step in range(diffusion_steps):
        ...
        cur_step = diffusion_steps - step
        use_sc = np.array([0 if step == 0 else 1], dtype=np.int64)
        temperature = t_min + (t_max - t_min) * cur_step / diffusion_steps
...
def _commit_canvas(tokens):
    ...
    commit_start = _next_encoder_position()
    ...
    commit_position_ids[:, :commit_len] = np.arange(commit_start, commit_start + commit_len, dtype=np.int64)
    ...
    session.run(_session_feed({
        ...
        "decoder_input_ids": np.zeros((1, canvas_length), dtype=np.int64),
        "decoder_position_ids": np.arange(commit_start, commit_start + canvas_length, dtype=np.int64).reshape(1, -1),
        "self_conditioning_logits": np.zeros((1, canvas_length, vocab_size), dtype=np.float32),
        "is_encode": np.array([1], dtype=np.int64),
    }))
    position_ids = commit_position_ids
...
for block in range(num_blocks):
    ...
    block_tokens, steps_run, canvas_time, accepted_count = _denoise_canvas(block)
    ...
    next_position = _next_encoder_position() + block_tokens.shape[1]
    print(
        f"  block {block + 1:2d} done: {steps_run} steps, {canvas_time:.1f}s, "
        f"accepted={accepted_count}/{canvas_length}, finalized={block_tokens.shape[1]}, next_position={next_position}"
    )
    if hit_eos:
        print(f"  EOS at block {block + 1}, offset {eos_pos}; truncating and stopping.")
        break
    if block + 1 < num_blocks and block_tokens.shape[1] > 0:
        _commit_canvas(block_tokens)
