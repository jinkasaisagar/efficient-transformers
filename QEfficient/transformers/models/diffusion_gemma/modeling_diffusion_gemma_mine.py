...
class QEffDiffusionGemmaModel(DiffusionGemmaModel):
    def forward(
        ...
        decoder_attention_mask: Optional[torch.Tensor] = None,
        decoder_position_ids: Optional[torch.LongTensor] = None,
        use_encoder: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> DiffusionGemmaModelOutputWithPast:
        ...
        if input_ids is None:
            raise ValueError("`input_ids` must be provided for ONNX-friendly forward.")
        if past_key_values is None:
            raise ValueError("`past_key_values` must be provided for ONNX-friendly forward.")
        if decoder_input_ids is None:
            raise ValueError("`decoder_input_ids` must be provided for ONNX-friendly forward.")

        if use_encoder is None:
            use_encoder = torch.ones((1,), dtype=torch.bool, device=input_ids.device)
        use_encoder = use_encoder.to(torch.bool)
        cond_hidden = use_encoder.view(1, 1, 1)
        cond_kv = use_encoder.view(1, 1, 1, 1)
        ...
        # )
        #
        # encoded_cache = encoder_outputs.past_key_values
        # if isinstance(encoded_cache, Cache):
        #     encoded_cache = encoded_cache.to_legacy_cache()
        # if isinstance(past_key_values, Cache):
        #     past_key_values = past_key_values.to_legacy_cache()
        #
        # past_key_values = tuple(
        #     (
        #         torch.where(use_encoder, enc_k, old_k),
        #         torch.where(use_encoder, enc_v, old_v),
        #     )
        #     for (old_k, old_v), (enc_k, enc_v) in zip(past_key_values, encoded_cache)
        # )
        #
        # encoder_last_hidden_state = encoder_outputs.last_hidden_state

        # Existing code kept for reference (torch.cond path):
        # if isinstance(past_key_values, Cache):
        #     past_key_values = past_key_values.to_legacy_cache()
        #
        # flat_past = tuple(t for layer in past_key_values for t in layer)
        # num_layers = len(past_key_values)
        # hidden_size = self.config.text_config.hidden_size
        #
        # def _encode_branch(old_flat):
        #     encoder_outputs = self.encoder(
        #         input_ids=input_ids,
        #         attention_mask=attention_mask,
        #         past_key_values=past_key_values,
        ...
        #         encoded_cache = encoded_cache.to_legacy_cache()
        #     flat_encoded = []
        #     for k, v in encoded_cache:
        #         flat_encoded.extend([k, v])
        #     return (encoder_outputs.last_hidden_state, *flat_encoded)
        #
        # def _skip_encode_branch(old_flat):
        #     zero_hidden = torch.zeros(
        #         (input_ids.shape[0], input_ids.shape[1], hidden_size),
        #         dtype=self.encoder.embed_tokens.weight.dtype,
        #         device=input_ids.device,
        #     )
        #     return (zero_hidden, *old_flat)
        #
        # cond_outputs = torch.cond(
        #     use_encoder.reshape(()),
        #     _encode_branch,
        #     _skip_encode_branch,
        #     (flat_past,),
        # )
        # encoder_last_hidden_state = cond_outputs[0]
        # selected_flat = cond_outputs[1:]
        # past_key_values = tuple((selected_flat[2 * i], selected_flat[2 * i + 1]) for i in range(num_layers))
        ...
        # encoded_cache = encoder_outputs.past_key_values.to_legacy_cache()
        # past_key_values = torch.where(use_encoder>0, encoded_cache, past_key_values)
        ...
        decode_ret = DiffusionGemmaModelOutputWithPast(last_hidden_state=decoder_outputs.last_hidden_state)
        encode_ret = DiffusionGemmaModelOutputWithPast(last_hidden_state=encoder_outputs.last_hidden_state, past_key_values=past_key_values)
        out = torch.where(use_encoder>0, encode_ret, decode_ret)
        # if isinstance(encoded_cache, Cache):
        #     encoded_cache = encoded_cache.to_legacy_cache()
        # if isinstance(past_key_values, Cache):
        #     past_key_values = past_key_values.to_legacy_cache()
        ...
        # past_key_values = tuple(
        #     (
        #         torch.where(cond_kv, enc_k, old_k),
        #         torch.where(cond_kv, enc_v, old_v),
        #     )
        #     for (old_k, old_v), (enc_k, enc_v) in zip(past_key_values, encoded_cache)
        # )
        # zero_hidden = torch.zeros_like(encoder_outputs.last_hidden_state)
        # encoder_last_hidden_state = torch.where(cond_hidden, encoder_outputs.last_hidden_state, zero_hidden)
...
class QEffDiffusionGemmaForBlockDiffusion(DiffusionGemmaForBlockDiffusion):
    ...
    def get_dummy_inputs(
        ...
        inputs = {
            ...
            "self_conditioning_mask": torch.ones((bs,), dtype=torch.bool),
            "use_encoder": torch.ones((1,), dtype=torch.bool),
    ...
    def get_onnx_dynamic_axes(
        ...
        dynamic_axes = {
            ...
            "self_conditioning_mask": {0: "batch_size"},
            "use_encoder": {0: "use_encoder_flag"},
        }
        if continuous_batching:
